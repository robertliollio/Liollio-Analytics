...
def fetch_standings(season):
    """Fetch official Euroleague standings."""
    try:
        s = Standings(COMPETITION)
        # get_standings requires a round_number — fetch the latest available round
        # Try rounds from high to low until we get data
        df = None
        for round_num in range(34, 0, -1):
            try:
                df = s.get_standings(season, round_num)
                if df is not None and not df.empty:
                    print(f"  Got standings for round {round_num}.")
                    break
            except Exception:
                continue
        ...
        for _, row in df.iterrows():
            standings.append({
                ...
                "team":     row.get("Club", row.get("Team", None)),
                "gp":       row.get("GamesPlayed", None),
                "wins":     row.get("Wins", None),
                "losses":   row.get("Losses", None),
                "pct":      row.get("WinPercentage", None),
                "pts_for":  row.get("PointsFor", None),
                "pts_against": row.get("PointsAgainst", None),
                "diff":     row.get("PointsDiff", None),
                "streak":   row.get("Streak", None),
            })
        return standings
    except Exception as e:
