...
def fetch_season_games(season):
    ...
    # ── Fetch player data from nested game stats ──────────────────────────────
    print(f"  Fetching season game stats with player data for season {season}...")
    player_games = pd.DataFrame()
    try:
        all_games_df = gs.get_game_stats_single_season(season)
        if all_games_df is not None and not all_games_df.empty:
            print(f"  Got {len(all_games_df)} game rows. Extracting player data...")
            player_rows = []
            for _, game_row in all_games_df.iterrows():
                gamecode = game_row.get("Gamecode", None)
                for team_col in ["local.players", "road.players"]:
                    players = game_row.get(team_col, [])
                    ...
                    for p in players:
                        try:
                            ...
                            club   = p["player"]["club"]
                            stats  = p["stats"]
                            if stats.get("timePlayed", 0) == 0:
                                continue
                            player_rows.append({
                                "Name":                person.get("name", ""),
                                "Team":                club.get("name", ""),
                                "Gamecode":            gamecode,
                                "Phase":               game_row.get("Phase", None),
                                "Score":               stats.get("points", 0),
                                "FieldGoalsMade2":     stats.get("fieldGoalsMade2", 0),
                                "FieldGoalsAttempted2": stats.get("fieldGoalsAttempted2", 0),
                                "FieldGoalsMade3":     stats.get("fieldGoalsMade3", 0),
                                "FieldGoalsAttempted3": stats.get("fieldGoalsAttempted3", 0),
                                ...
                                "TotalRebounds":       stats.get("totalRebounds", 0),
                                "Assistances":         stats.get("assistances", 0),
                                "Steals":              stats.get("steals", 0),
                                "Turnovers":           stats.get("turnovers", 0),
                                "BlocksFavour":        stats.get("blocksFavour", 0),
                                "BlocksAgainst":       stats.get("blocksAgainst", 0),
                                "FoulsCommited":       stats.get("foulsCommited", 0),
                                "FoulsReceived":       stats.get("foulsReceived", 0),
            ...
            if player_rows:
                ...
                print(f"  Extracted {len(player_games)} player-game rows.")
            else:
                print(f"  No player rows extracted.")
        else:
            print(f"  No game stats returned.")
    except Exception as e:
...
def calculate_player_stats(player_games):
    """
    Aggregate per-game player box scores into season totals.
    Input: DataFrame with one row per player per game.
    Columns: Name, Team, Gamecode, Score, FieldGoalsMade2, etc., TimePlayed (seconds)
    """
    ...
    # Map column names from the nested JSON extraction
    col_map = {
        "Score":               "PTS",
        ...
        "FoulsCommited":       "PF",
        "FoulsReceived":       "FD",
        "TimePlayed":          "SEC",  # seconds
    }
    df = df.rename(columns={k: v for k, v in col_map.items() if k in df.columns})

    if "Name" not in df.columns or "Team" not in df.columns:
        print(f"  WARNING: Missing Name/Team columns. Available: {df.columns.tolist()}")
        return []

    # Convert seconds to minutes
    df["MIN"] = pd.to_numeric(df.get("SEC", 0), errors="coerce").fillna(0) / 60

    # Ensure numeric stat columns
    num_cols = ["PTS","FGM2","FGA2","FGM3","FGA3","FTM","FTA",
                "OREB","DREB","TREB","AST","STL","TOV","BLK","BLKA","PF","FD"]
    for col in num_cols:
    ...
    print(f"  Player rows after filtering DNPs: {len(df)}")

    # Get all teams per player in order played (semicolon-separated if multiple)
    if "Gamecode" in df.columns:
        def all_teams(s):
            seen = []; 
            [seen.append(t) for t in s if t not in seen]
            return ";".join(seen)
        team_history = df.sort_values("Gamecode").groupby("Name")["Team"].apply(all_teams).reset_index()
        team_history.columns = ["Name", "AllTeams"]
        latest_team = team_history
    else:
        latest_team = None

    # Aggregate season totals per player
    agg_cols = {c: "sum" for c in num_cols + ["MIN"] if c in df.columns}
    agg_cols["Gamecode"] = "nunique"

    totals = df.groupby("Name").agg(agg_cols).reset_index()
    totals.rename(columns={"Gamecode": "GP"}, inplace=True)

    # Attach team history
    if latest_team is not None:
        totals = totals.merge(latest_team, on="Name", how="left")
        totals["Team"] = totals["AllTeams"].fillna("Unknown")

    print(f"  Aggregated {len(totals)} players.")
    ...
    for _, p in totals.iterrows():
        ...
        # PIR per game (from season totals / GP)
        missed_fg = fga - fgm
        missed_ft = fta - ftm
        pir_total = (pts + treb + ast + stl + blk + fd) - (missed_fg + missed_ft + tov + blka + pf)
        pir_pg    = safe_div(pir_total, gp)

        min_pg  = safe_div(mins, gp)
        pct_min = safe_div(min_pg * 100, 40)
        efg     = safe_div((fgm + 0.5 * fgm3) * 100, fga)
        ts      = safe_div(safe_div(pts, gp) * 100,
                           2 * (safe_div(fga, gp) + 0.475 * safe_div(fta, gp)))
        fg3pct  = safe_div(fgm3 * 100, fga3)
        fg2pct  = safe_div(fgm2 * 100, fga2)
        ftpct   = safe_div(ftm  * 100, fta)

        player_stats.append({
            "name":    str(p.get("Name", "")),
            "team":    str(p.get("Team", p.get("AllTeams", "Unknown"))),
            "gp":      int(gp),
            "min_pg":  round1(min_pg),
            "pir":     round1(pir_pg),
            "pct_min": round1(pct_min),
            "efg":     round1(efg),
            "ts":      round1(ts),
            "fg3m_pg": round1(safe_div(fgm3, gp)),
            "fg3a_pg": round1(safe_div(fga3, gp)),
            "fg3_pct": round1(fg3pct),
            "fg2m_pg": round1(safe_div(fgm2, gp)),
            "fg2a_pg": round1(safe_div(fga2, gp)),
            "fg2_pct": round1(fg2pct),
            "ftm_pg":  round1(safe_div(ftm, gp)),
            "fta_pg":  round1(safe_div(fta, gp)),
            "ft_pct":  round1(ftpct),
            "pts_pg":  round1(safe_div(pts, gp)),
            "reb_pg":  round1(safe_div(treb, gp)),
            "ast_pg":  round1(safe_div(ast, gp)),
            "stl_pg":  round1(safe_div(stl, gp)),
            "blk_pg":  round1(safe_div(blk, gp)),
            "tov_pg":  round1(safe_div(tov, gp)),
        })
...
def fetch_standings(season):
    ...
    The Standings API returns empty data so we derive from game results.
    """
    try:
        gm = GameMetadata(COMPETITION)
        meta_df = gm.get_gamecodes_season(season)
        ...
        if not all([home_col, away_col, hscore_col, ascore_col, played_col]):
            print(f"  Missing schedule columns for standings.")
            return []

        teams = {}
        for _, row in meta_df.iterrows():
            if not row[played_col]:
                continue
            home = row[home_col]
            away = row[away_col]
            try:
...
def run_pipeline(season):
    ...
    # ── Calculate stats — full season + per phase ───────────────────────────
    PHASES = {
        'all': None,
        'rs':  'RS',
        'pi':  'PI',
    ...
    for phase_key, phase_val in PHASES.items():
        if phase_val is None:
            games_slice = team_games
        else:
            if 'Phase' not in team_games.columns:
                continue
            games_slice = team_games[team_games['Phase'] == phase_val]
        if games_slice.empty:
            continue
        stats = calculate_team_stats(games_slice)
        if stats:
            team_stats_by_phase[phase_key] = stats
            print(f"  [{phase_key.upper()}] {len(stats)} teams.")

    team_stats = team_stats_by_phase.get('all', [])
    print(f"  Calculated stats for {len(team_stats)} teams (all phases).")

    print(f"[{season_label(season)}] Calculating player stats...")
    player_stats_by_phase = {}
    if player_games is not None and not player_games.empty:
        for phase_key, phase_val in PHASES.items():
            if phase_val is None:
                pg_slice = player_games
            else:
                if 'Phase' not in player_games.columns:
                    continue
                pg_slice = player_games[player_games['Phase'] == phase_val]
            if pg_slice.empty:
                continue
            stats = calculate_player_stats(pg_slice)
            if stats:
                player_stats_by_phase[phase_key] = stats
                print(f"  [{phase_key.upper()}] {len(stats)} players.")

    player_stats = player_stats_by_phase.get('all', [])
    print(f"  Calculated stats for {len(player_stats)} players (all phases).")
    ...
    write_json(os.path.join(out_dir, "teams.json"), {
        ...
        "teams": team_stats,
        "phases": {k: v for k, v in team_stats_by_phase.items() if k != 'all'}
    })
    ...
    write_json(os.path.join(out_dir, "players.json"), {
        ...
        "phases": {k: v for k, v in player_stats_by_phase.items() if k != 'all'}
    })
